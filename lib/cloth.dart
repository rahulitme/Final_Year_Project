...
enum ColorOption { Recommended, Check, Plan }
...
class ClothingColorRecommender extends StatefulWidget {
  ...
  @override
  _ClothingColorRecommenderState createState() =>
      _ClothingColorRecommenderState();
}
...
class _ClothingColorRecommenderState extends State<ClothingColorRecommender> {
      ...
      if (decodedImage != null) {
        ...
        setState(() {
          ...
          recommendedColors = getRecommendedColors(
              skinToneCategory, selectedGender, selectedColorOption);
        });
  ...
  List<Map<String, Color>> getRecommendedColors(String skinTone, String gender,
      [ColorOption option = ColorOption.Recommended]) {
    switch (option) {
  ...
  List<Map<String, Color>> _getRecommendedColors(
      String skinTone, String gender) {
    switch (skinTone) {
      case 'Fair':
        return gender == 'Male'
            ? [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Clothing Color Recommender'),
            ...
            if (isValid)
              Column(
                children: [
                  ...
                  SizedBox(height: 10),
                  Text('Skin Tone Category: $skinToneCategory',
                      style: TextStyle(fontSize: 16)),
                  SizedBox(height: 20),
                  Text(
                      '${selectedColorOption.toString().split('.').last} Colors:',
                      style:
                          TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  SizedBox(height: 10),
                  Wrap(
